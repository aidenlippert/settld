...
export class DedupeStore {
  ...
  listPendingAcks({ nowMs = Date.now(), limit = 50 } = {}) {
    ...
    for (const r of this.records.values()) {
      ...
      const nextAt = r.ackNextAttemptAt ? Date.parse(r.ackNextAttemptAt) : null;
      if (nextAt && Number.isFinite(nextAt) && nextAt > nowMs) continue;
      out.push(r);
      if (out.length >= limit) break;
    }
    out.sort((a, b) => String(a.ackNextAttemptAt ?? "").localeCompare(String(b.ackNextAttemptAt ?? "")));
    return out;
  }
}
