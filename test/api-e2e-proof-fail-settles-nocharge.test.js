...
test("API e2e: strict mode allows FAIL settlement (no-charge refund), amountCents=0 in export", async () => {
  ...
  const contractRes = await request(api, {
    method: "POST",
    path: "/ops/contracts",
    body: { contractId: "contract_strict_fail", name: "Strict Proof Contract", policies: { proofPolicy: { gateMode: "strict" } } }
  });
  ...
  const quote = await request(api, {
    ...
    body: { startAt: bookingStartAt, endAt: bookingEndAt, environmentTier: "ENV_MANAGED_BUILDING", requiresOperatorCoverage: false, contractId: "contract_strict_fail" }
  });
  ...
  const book = await request(api, {
    ...
    body: {
      ...
      contractId: "contract_strict_fail"
    }
  });
  ...
  const postRobotEvent = async (type, payload, at) => {
    const draft = createChainedEvent({ streamId: jobId, type, actor: { type: "robot", id: "rob_fail" }, payload, at });
  ...
  await postServerEvent("MATCHED", { robotId: "rob_fail" }, `m_${jobId}`);
  await postServerEvent("RESERVED", { robotId: "rob_fail", startAt: bookingStartAt, endAt: bookingEndAt, reservationId: `rsv_${jobId}` }, `r_${jobId}`);
  ...
  await postRobotEvent(
    ...
    { jobId, zoneId: "zone_default", coveragePct: 10, window: { startAt: bookingStartAt, endAt: bookingEndAt }, source: "robot" },
    nowIso()
  );
  ...
  const line = text
    .split("\n")
    .find((l) => l.startsWith(`${jobId},cust_x,site_x,reset_lite,`));
  assert.ok(line);
  const cols = line.split(",");
  assert.equal(Number(cols[7]), 0); // grossAmountCents
  assert.equal(Number(cols[11]), 0); // netDueCents
});
