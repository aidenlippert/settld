...
test("API e2e: strict mode rejects settlement on stale proof factsHash until proof is re-evaluated", async () => {
  ...
  const contractRes = await request(api, {
    method: "POST",
    path: "/ops/contracts",
    body: { contractId: "contract_strict_stale", name: "Strict Proof Contract", policies: { proofPolicy: { gateMode: "strict" } } }
  });
  ...
  const quote = await request(api, {
    ...
    body: { startAt: bookingStartAt, endAt: bookingEndAt, environmentTier: "ENV_MANAGED_BUILDING", requiresOperatorCoverage: false, contractId: "contract_strict_stale" }
  });
  ...
  const book = await request(api, {
    ...
    body: {
      ...
      contractId: "contract_strict_stale"
    }
  });
  ...
  const postRobotEvent = async (type, payload, at) => {
    const draft = createChainedEvent({ streamId: jobId, type, actor: { type: "robot", id: "rob_stale" }, payload, at });
  ...
  await postServerEvent("MATCHED", { robotId: "rob_stale" }, `m_${jobId}`);
  await postServerEvent("RESERVED", { robotId: "rob_stale", startAt: bookingStartAt, endAt: bookingEndAt, reservationId: `rsv_${jobId}` }, `r_${jobId}`);
  ...
  // Late evidence after proof: changes facts (and should require re-proof).
  nowMs += 30_000;
  await postRobotEvent(
    "ZONE_COVERAGE_REPORTED",
    { jobId, zoneId: "zone_default", coveragePct: 10, window: { startAt: bookingStartAt, endAt: "2026-01-20T11:00:01.000Z" }, source: "robot" },
    nowIso()
  );

  nowMs = Date.parse("2026-01-20T12:00:00.000Z");
  const settleStale = await request(api, {
    method: "POST",
    path: `/jobs/${jobId}/events`,
    headers: { "x-proxy-expected-prev-chain-hash": lastChainHash, "x-idempotency-key": `s_${jobId}_stale` },
    body: { type: "SETTLED", actor: { type: "system", id: "proxy" }, payload: { settlement: "demo" } }
  });
  assert.equal(settleStale.statusCode, 400);
  assert.equal(settleStale.json?.code, "PROOF_STALE");
